...
function finitedifference(q::Array{Float64, 3}, u::Array{Float64, 3}, Δt::Float64, g::Geometry)
    # (Nx_, Ny_) = size(q)
    q_ = similar(q)
    for i in 2:g.Nx, j in 2:g.Ny        ## Should be able to go from 1 to Nx+1
        ...
        vec_x =  dot(u[i,j,:], [partial_x[1] partial_y[1]])
        vec_y =  dot(u[i,j,:], [partial_x[2] partial_y[2]])
        q_[i,j,:] = q[i,j,:] - Δt * [vec_x, vec_y]
    end
    return q_
end
